...
def inicializar_banco():
    try:
        criar_tabelas()
        if not buscar_usuario('cotead'):
            adicionar_usuario('cotead', '2023Cote@d', 'admin')
...
def index():
    return render_template('agenda.html', cargo=session.get('cargo'))
...
def deletar_usuario(id):
    ...
    try:
        ...
        # Contar número de admins e cotead (ambos têm acesso total)
        cursor.execute('SELECT COUNT(*) FROM Users WHERE cargo IN ("admin", "cotead")')
        admin_count = cursor.fetchone()[0]

        # Impedir exclusão do último admin/cotead
        if cargo in ('admin', 'cotead') and admin_count <= 1:
            cursor.execute('ROLLBACK')
